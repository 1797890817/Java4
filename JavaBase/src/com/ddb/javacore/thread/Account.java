...
public class Account {
	 ...
	 */
	public synchronized void deposite(double depositeAmount, int j) {
		
		if (hasMoney) {
			// 账户中已有人存钱进去，此时当前线程需要等待阻塞
			try {
				System.out.println(Thread.currentThread().getName() + " 开始要执行wait操作" + " -- j=" + j);
				wait();
			} catch (InterruptedException e) {
				e.printStackTrace();
			}
			...
			// 开始存钱
			System.out.println(Thread.currentThread().getName() + " 存款:" + depositeAmount + " -- j=" + j);
